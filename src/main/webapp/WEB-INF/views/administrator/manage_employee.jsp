...
<body>
	...
	<div class="page-container">
					...
					<div class="col-12 mt-3">
						<div class="card">
							<div class="card-body">
								...
								<form action="${base}/admin/employee/manage_employees"
									method="get">
									<div
										class="d-flex flex-row mb-3 search-form justify-content-between">
										<a href="/admin/employee/addOrEdit_employee">
									...
									<div class="table-responsive">
										<table class="table text-center">
											...
											<tbody style="line-height: 100px !important;">
												...
												<c:forEach items="${employees.data}" var="employees"
													varStatus="loop">
													<tr>
														<td>${stt}</td>
														<td>${employees.id}</td>
														...
														<c:choose>
															<c:when test="${employees.status==false}">
																<td><span class="false_status">Vô hiệu hóa</span></td>

																<td><a
																	href="${base }/admin/employee/edit_employee?id=${employees.id}"><i
																		class="bi bi-pencil-square "></i> </a> <a> <i
																		class="bi bi-person-check-fill" role="button"
																		data-toggle="modal"
																		data-target="#modalActive-${employees.id}"></i>
																</a></td>
															</c:when>
															<c:otherwise>
																...
																<td><a
																	href="${base }/admin/employee/edit_employee?id=${employees.id}"><i
																		...
																		data-target="#modalDelete-${employees.id}"></i>
																</a></td>
															</c:otherwise>
														</c:choose>
													...
													<!--Modal xóa  -->
													<div class="modal fade" id="modalDelete-${employees.id}">
															...
															<div class="modal-content">
																...
																<div class="modal-body">
																	<p>
																		Bạn có chắc chắn muốn vô hiệu hóa tài khoản <span
																			style="color: red">${employees.username}</span> khỏi
																		hệ thống
																	</p>
																</div>
																...
																<div class="modal-footer">
																	...
																	<button type="button" class="btn btn-primary"
																		onclick="disableEmployee(${employees.id})">Vô
																		hiệu hóa</button>
																</div>
															</div>
														</div>
													...
													<!--Modal kích hoạt  -->
													<div class="modal fade" id="modalActive-${employees.id}">
														<div class="modal-dialog modal-dialog-centered"
															role="document">
															<div class="modal-content">
																<div class="modal-header">
																...
																<div class="modal-body">
																	<p>
																		Bạn có chắc chắn muốn kích hoạt tài khoản <span
																			style="color: red">${employees.username}</span>
																	</p>
																</div>
																...
																<div class="modal-footer">
																	<button type="button" class="btn btn-secondary"
																		data-dismiss="modal">Hủy</button>
																	<button type="button" class="btn btn-primary"
																		onclick="activeEmployee(${employees.id})">Kích
																		hoạt</button>
																</div>
															</div>
														</div>
	 ...
	 function disableEmployee(employeeId) {
			...
			jQuery.ajax({
				url: "${base}/admin/employee/manage_employees/delete", 	   	   
				type: "delete",					 
	 ...
	 function activeEmployee(employeeId) {
			...
			jQuery.ajax({
				url: "${base}/admin/employee/manage_employees/active", 	   	   
				type: "post",					 
				contentType: "application/json",   
				data: JSON.stringify(data), 
		...
		$(document).ready(function() {
			...
			$("#paging").pagination({
				currentPage:${employees.currentPage}, //trang hiện tại
				items : ${employees.totalItems},
				itemsOnPage : ${employees.sizeOfPage},
